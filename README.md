...
A股 / 港股 / 美股 / 全球 / 大宗商品·汇率 各板块表格新增 **「年内」列**，展示各标的自年初以来的涨跌幅。

**计算方式**：`年内涨跌幅 = (当前价 / 年初基数 - 1) × 100%`

**年初基数自动获取**（每年首次运行自动生成，跨年自动重置，零人工维护）：
- 每年首次运行从**日K线**自动提取**当年首个交易日收盘价**作为基数，存入 KV（key 含年份，如 `ytd_base_2026`）
- 之后每次运行读 KV 计算，不重复请求
...
| A股（8只） | 新浪日K |
| 港股 / 全球 / 汇率 | 东方财富日K |
| 美股（指数/ETF） | Yahoo `range=1y` |
| 美股期货（2只）/ 大宗商品 | 暂未接入 → 该列显示 `—` |
