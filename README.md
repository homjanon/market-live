...
| 规则 | 值 |
|---|---|
| **Cron**（UTC） | `15,45 1-8 * * 1-5` |
| **Cron**（北京） | `9:15, 9:45, 10:15, …, 16:15, 16:45`（每 30 分钟） |
| **首次触发** | 北京 **9:15** |
| **末次触发** | 北京 **16:45** |
| **交易日** | **A 股交易日**（腾讯财经接口 `is_tx_today()` 校验当日是否为交易日，含调休） |
| **周末** | ❌ 跳过 |
| **节假日** | ❌ 跳过（春节/国庆/清明等） |

双重守卫：
- `in_trading_window()` — 北京时 9:30–16:15、周一~周五（因此 Cron 的 9:15 / 16:45 两拍被跳过，实际取数窗口约 9:45–16:15）
- `is_tx_today()` — 通过腾讯财经接口校验当日是否为交易日（**原 legu 校验方案已弃用**）
