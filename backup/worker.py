...
INDEX_MAP = {
    ...
    "科创50":     ("1.000688", "A"),
    "北证50":     ("0.899050", "A"),
    "红利低波ETF易方达": ("1.563020", "A"),
    "30年国债ETF博时": ("1.511130", "A"),
    "恒生指数":   ("100.HSI",  "H"),
    "恒生科技":   ("124.HSTECH", "H"),
    "恒生国企指数": ("100.HSCEI", "H"),
    "日经225":    ("100.N225",  "G"),
...
GROUPS = {
    "A":  ["上证指数", "深证成指", "创业板指", "沪深300", "科创50", "北证50", "红利低波ETF易方达", "30年国债ETF博时"],
    "H":  ["恒生指数", "恒生科技", "恒生国企指数"],
...
async def fetch_us_quotes():
    """美股行情：指数/ETF(雅虎) + 股指期货(新浪hf_)，按指定顺序输出。单条失败降级为None。"""
    YAHOO = [
        ("标普500", "^GSPC"),
        ("纳斯达克100", "^NDX"),
        ("纳斯达克综合", "^IXIC"),
    ...
    ORDER = [n for n, _ in YAHOO] + [n for n, _ in SINA]

    async def _yq(name, sym):
        # 雅虎：^ 开头的指数为指数，无 ^ 为 ETF（SOXX/SCHD）
        url = f"https://query1.finance.yahoo.com/v8/finance/chart/{sym.replace('^', '%5E')}?interval=1d&range=1d"
        try:
            r = jload(await http_get(url, ref="https://finance.yahoo.com"))
            meta = (r.get("chart") or {}).get("result", [{}])[0].get("meta", {})
    ...
    async def _sina():
        out = {}
        try:
            syms = ",".join(s for _, s in SINA)
            txt = await http_get(f"https://hq.sinajs.cn/list={syms}", ref="https://finance.sina.com.cn")
            for line in txt.split(";"):
                line = line.strip()
                if not line or "=" not in line:
                    continue
                key, val = line.split("=", 1)
                sym = key.replace("var hq_str_", "").strip()
                nm = {s: n for n, s in SINA}.get(sym)
                if not nm:
                    continue
                parts = [p.strip() for p in val.strip('"').split(",")]
                try:
                    ...
                    last, chg = None, None
                out[nm] = {"name": nm, "price": last, "chg": chg, "source": "sina hf_"}
        except Exception:
            pass
        return out

    yahoo_results, sina_results = await asyncio.gather(
        asyncio.gather(*[_yq(n, s) for n, s in YAHOO]),
        _sina(),
    )
    ydict = dict(yahoo_results)
    out = []
    for n in ORDER:
        out.append(ydict.get(n) if n in ydict else sina_results.get(n))
    return out
...
async def build_snapshot(env):
    ...
    (indices, commodities, hs300, vol_mult, breadth, fund,
     idx_chg, etf, valuation, us_fear, us_quotes) = await asyncio.gather(
