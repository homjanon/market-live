...
**核心逻辑**：Worker 每30分钟采集 11 路数据源 → 计算 XXFI + 冰点 → 同时写入 KV（供 VPN 版读取）和推送到 GitHub Pages（供国内直连）。
...
| ④ 兜 | Yahoo `v8 chart` | 除北证50、红利低波外全覆盖（含斯托克600） | 按缺口，每项 1 |
...
- ⚠️ **红利低波无兜底**：全网仅东财有 `2.H30269`。失败即**显式缺失**，不拿 `sh000069`（消费80）、`sz399321`（国证红利）等相近指数冒充。
...
│   ├─ 采集 11 路数据源    # (HTTP 并发, asyncio.gather)
...
| 来源 | 用途 | 协议 |
|---|---|---|
| **东方财富** push2delay | 指数/汇率 **主源**（`ulist.np` 批量，1 次请求拿全 20 项）、资金流、ETF 篮子、K-line（成交额） | 公开 HTTP API |
