...
各表格的「年内」列 = `(当前价 / 当年首个交易日收盘 − 1) × 100%`，年初基数一年不变、随当前价实时变化。

| 分组 | 年初基数来源 | 说明 |
|---|---|---|
| A股 | 新浪 A股日K | 2026 起已移除 sh000069（那是另一指数），红利低波改用东财 2.H30269，与实时价同源 |
| 港股/全球/汇率 | 东财日K（push2his） | 海外 secid 偶发断连，重试 5 次 + 静态表兜底 |
| 美股（指数+ETF） | 新浪美股日K | 稳定替代 Yahoo（数据完全一致）；新浪无期货代码，美股期货不显示年内 |
| 兜底 | `YTD_BASE_STATIC` 静态表 | 动态抓取失败时使用已核实的 2026 年初基数（如斯托克600=596.14），保证年内始终可显示 |

**缓存机制**：KV 年key（`ytd_base_{年份}`），当年首个交易日抓取后整表缓存、后续缺失项增量补抓；跨年自动换 key 重新抓取，无需手动维护。
