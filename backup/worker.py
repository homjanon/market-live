...
YTD_EM_KL = {
    ...
    "法国CAC40": "100.FCHI", "英国富时100": "100.FTSE",
    "美元离岸人民币": "133.USDCNH",
    "红利低波": "2.H30269",   # 与实时价同源（新浪sh000069是另一指数）
    # 欧洲斯托克600 已移出动态源：东财K线时有时无且值不可靠（580.84 vs 用户核实596.14），仅用静态表
}
# 美股YTD基数源：新浪美股日K（稳定，替代Yahoo。新浪不支持期货，期货不启用年内）
YTD_US_SINA = {
    "标普500": ".INX", "纳斯达克100": ".NDX", "纳斯达克综合": ".IXIC",
    "道琼斯": ".DJI", "美国红利指数ETF(SCHD)": "SCHD", "半导体ETF": "SOXX",
}
YTD_NO_BASE = {"WTI原油", "COMEX黄金", "布伦特原油", "白银", "30年国债期货"}

# 年初基数静态表（兜底，仅动态抓取失败时生效）
# 已核实：港股/全球/美股 2026-01-02 收盘（日经01-05），多源交叉验证。
# 欧洲斯托克600 = 596.14（用户直连英为财情核实）。美股期货新浪无代码，不启用。
YTD_BASE_STATIC = {
    "2026": {
        "红利低波": 11235.18,          # 东财2.H30269（非新浪sh000069）
        "恒生指数": 26338.47, "恒生科技": 5736.44, "恒生国企指数": 9168.99,
        "日经225": 51832.80, "韩国KOSPI": 4309.63, "德国DAX": 24539.34,
        "欧洲斯托克600": 596.14, "法国CAC40": 8195.21, "英国富时100": 9951.14,
        "美元离岸人民币": 6.9799,      # 东财首条=01-01（离岸元旦不休市），非01-02的6.9697
        "标普500": 6858.47, "纳斯达克100": 25206.17, "纳斯达克综合": 23235.63,
        "道琼斯": 48382.39, "美国红利指数ETF(SCHD)": 27.73, "半导体ETF": 313.69,
    },
}
...
def _parse_sina_us_jsonp(text):
    """新浪美股日K返回 JSONP（/*..*/ var x=([{...},...]);），剥离前缀后解析。"""
    m = re.search(r"var x=\((\[.*\])\);", text, re.S)
    if not m:
        return None
    try:
...
async def fetch_sina_us_ytd_base(symbol):
    """新浪美股日K：支持指数(.IXIC/.INX/.DJI/.NDX)与ETF(SCHD/SOXX)。
    响应约300-600KB全量历史，仅当年KV缺失时调用一次（年key缓存），可接受。"""
    url = (f"https://stock.finance.sina.com.cn/usstock/api/jsonp_v2.php/"
           f"var%20x=/US_MinKService.getDailyK?symbol={symbol}&___qn=3")
    text = await http_get(url, ref="https://finance.sina.com.cn")
    data = _parse_sina_us_jsonp(text)
    if not isinstance(data, list) or not data:
        return None
    y = _ytd_year()
    for r in data:
        if r.get("d", "").startswith(y):
            try:
                return float(r["c"])
...
async def load_ytd_bases(env):
    """年内基数：年key缓存 + 增量填充 + 静态表兜底。
    1) KV ytd_base_{年份} 已有 → 仅缺失项补抓（增量）；
    2) 动态抓取（新浪A股/东财/新浪美股）成功后合并；
    3) 仍缺失且有静态表值 → 用静态值兜底（保证美股/全球显示）；
    4) 写回 KV（年key），跨年自动换 key，2027 首个交易日重新抓取。"""
    y = _ytd_year()
    kv_key = YTD_BASE_KV_PREFIX + y
    bases = {}
    try:
        raw = await env.KV.get(kv_key)
        if raw:
            d = jload(raw)
    ...
    for n, sym in YTD_US_SINA.items():
        if n not in bases:
            tasks.append(_one(n, fetch_sina_us_ytd_base, sym))
    if tasks:
        await asyncio.gather(*tasks)

    # 静态表兜底（不覆盖动态抓取值）
    stat = YTD_BASE_STATIC.get(y) or {}
    for n, v in stat.items():
        if n not in bases and v is not None:
            bases[n] = v

    # 写回KV，供当年后续运行直接读取
    try:
        await env.KV.put(kv_key, json.dumps(bases))
    except Exception:
        pass
    return bases
